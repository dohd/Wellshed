...
<script>
    ...
    const Form = {
        init(){
            ...
            $('#employee').select2();
            $('#budget').select2().attr('disabled',true);
            ...
            $('#submitmilestoneForm').click(function(e) {
                ...
                $.ajax({
                    url: "{{ route('biller.milestones.update', ['milestone'=>$milestone]) }}",
                    type: 'POST',
        ...
        productRow(v,i){
            return `
                <tr>
                    ...
                    <td><input type="text" class="form-control qty" name="qty[]" value="${+v.new_qty}" id="qty-p0" step="0.1" required disabled></td>
                    ...
                    <input type="hidden" name="budget_item_id[]" class="budget_item_id" value="${v.id}" disabled>
        ...
        selectItemChange(){
            ...
            if (el.is(':checked')) {
                ...
                row.find('.budget_item_id').prop('disabled', false);
                ...
                row.find('.budget_item_id').prop('disabled', true);
        ...
        qtyChange(){
            ...
            const previous_qty = accounting.unformat(row.find('.previous_qty').val());
            let acceptable_qty = qty_allocated_to_milestones - previous_qty;
            let new_qty = budget_qty - acceptable_qty;
