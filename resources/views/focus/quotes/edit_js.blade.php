...
<script>
    ...
    $('.datepicker').each(function() {
        ...
        .datepicker('setDate', d? new Date(d) : '');
    });
    ...
    // on currency change
    let initRate = $('#currency option:selected').attr('rate') * 1;
        ...
        $("#quoteTbl tbody tr").each(function(i) {
            ...
            if (qty > 0) {
                ...
                if (isMisc) {
                    ...
                    const estqty = accounting.unformat($(this).find('.estqty').val());
                    const taxrate = accounting.unformat($(this).find('.tax_rate').val());
                    v = 0;
                    if (taxrate === 0) {
                        v = buyprice;
        ...
        return {
            ...
            select: function(event, ui) {
                ...
                if (row.hasClass('misc')) {
                    ...
                    let buy_price = accounting.formatNumber($('#buyprice-' + i).val());
                    if(buy_price < 1){
    ...
    const quoteReference = @json(@$quote->reference);
    const equips = @json(@$quote->equipments);
    if (quoteReference) $('#attach-djc').prop("checked", true).change();
    else $('#attach-djc').prop("checked", false).change();
    if (equips && equips.length) $('#add-check').prop("checked", true).change();
    else $('#add-check').prop("checked", false).change();
