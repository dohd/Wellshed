<button type="button" class="btn btn-sm btn-primary" id="refreshButton">Numbering</button>
<table id="quoteTbl" class="table-responsive pb-5 tfr my_stripe_single">
    <thead>
        <tr class="bg-gradient-directional-blue white">
            ...
            <th width="10%" class="text-center est_purchase_price">Est. Buy Price</th>
    ...
    <tbody>
        ...
        <tr id="productRow">
            ...
            <td>
                <textarea name="product_name[]" id="name-p0" cols="35" rows="2" class="form-control pname" placeholder="{{trans('general.enter_product')}}" required></textarea>
            </td>
            ...
            @foreach ($quote->products as $k => $item)
                @if ($item->a_type == 1)
                    ...
                    <tr class="{{ !$item->misc ?: 'misc' }}" style="{{ $item->misc == 1 ? 'background-color:rgba(229, 241, 101, 0.4);' : '' }}">
                        <td><input type="text" class="form-control numbering" name="numbering[]" value="{{ $item->numbering }}" id="numbering-p{{$k}}"></td>
                        <td>
                        ...
                        <td><input type="text" class="form-control estqty" name="estimate_qty[]" value="{{ numberFormat($item->estimate_qty, 1) }}" id="estqty-p{{$k}}" step="0.1"  style="border:solid #f5a8a2;" required></td>  
                        ...
                        <td><input type="number" class="form-control qty {{ !$item->misc ?: 'invisible' }}" name="product_qty[]" value="{{ round($item->product_qty,1) }}" id="qty-p{{$k}}" step="0.1" {{ $item->misc ?: 'required' }}></td>
                        <td>
                            <input type="text" class="form-control rate {{ !$item->misc ?: 'invisible' }}" name="product_subtotal[]" value="{{ number_format($item->product_subtotal, 4) }}" id="rate-p{{$k}}" {{ $item->misc ?: 'required' }}>
                        </td>
                        <td>
                            <div class="row no-gutters">
                                <div class="col-6">
                ...
                @else
                    <!-- Title Row  -->
                    <tr>
                        <td><input type="text" class="form-control numbering" name="numbering[]" value="{{ $item->numbering }}" style="font-weight: bold;" id="numbering-t{{$k}}"></td>
                        <td colspan="8">
