            ...
            @foreach ($quote->products as $k => $item)
                @if ($item->a_type == 1)
                    ...
                    <tr class="{{ !$item->misc ?: 'misc' }}">
                        ...
                        <td><input type="number" step="0.1" class="form-control qty {{ !$item->misc ?: 'invisible' }}" name="product_qty[]" value="{{ $item->product_qty }}" id="qty-p{{$k}}" {{ $item->misc ?: 'required' }}></td>
                        <td><input type="text" class="form-control rate {{ !$item->misc ?: 'invisible' }}" name="product_subtotal[]" value="{{ numberFormat($item->product_subtotal) }}" id="rate-p{{$k}}" {{ $item->misc ?: 'required' }}></td>
