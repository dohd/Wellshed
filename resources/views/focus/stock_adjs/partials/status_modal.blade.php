<div class="modal fade" id="statusModal" tabindex="-1" role="dialog" aria-labelledby="statusModal" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content w-75">
            ...
            {{ Form::model($stock_adj, ['route' => ['biller.stock_adjs.approve_adjustment', $stock_adj], 'method' => 'POST' ]) }}
                <div class="modal-body">
                    <div class="form-group">
                        <label for="status">Approval Status</label>
                        <select class="form-control" name="approval_status" id="status">
                            @foreach (['Pending', 'Approved'] as $value)
                                <option value="{{ $value }}" {{ $value == $stock_adj->approval_status? 'selected' : '' }}>
                                    {{ $value }}
                                </option>
                            @endforeach
                        </select>
