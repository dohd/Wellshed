    ...
    <div class="card-content">
        <div class="card-body pb-0">
            ...
            <div class="row mb-1">
                <div class="col-2">                                        
                    ...
                    <div class="input-group">
                        @php
                            $quote = $boq->lead->quotes()->first() ?? '';
                            if($quote) {
                                $quote_id = $quote->id;
                                $project_id = $quote->project->id ?? '';
                                $project_tid = $quote->project ? $quote->project->tid : '';
                            }
                        @endphp
                        <input type="hidden" name="project_id" value="{{ $project_id }}" id="">
                        <input type="hidden" name="quote_id" value="{{ $quote_id }}" id="">
                        {{ Form::text('tid_prj', gen4tid('PRJ-', @$project_tid), ['class' => 'form-control', 'id' => 'tid', 'disabled']) }}
                    </div>
                </div>  
                <div class="col-2">                                        
                    <label for="serial_no" class="caption mb-0">BoQ No</label>
                    <div class="input-group">
                        @php $tid = gen4tid("BoQ-", $boq->tid); @endphp
                        {{ Form::text('tid', $tid . $boq->revision, ['class' => 'form-control', 'id' => 'tid', 'disabled']) }}
                    </div>
                </div>  
                <div class="col-3">
                    ...
                    <div class="input-group">
                        ...
                        {{ Form::text('client', @$name, ['class' => 'form-control round', 'id' => 'client', 'disabled']) }}
                        <input type="hidden" name="customer_id" value="{{ @$customer->id }}" id="client_id">
                ...
                <div class="col-5">
                    ...
                    {{ Form::text('notes', @$boq->name, ['class' => 'form-control', 'id'=>'subject', 'disabled']) }}
                </div>
            </div>
            ...
            <div class="row mb-1">
                ...
                <div class="col-10">
                    <label for="note" class="caption mb-0">Valuation Title</label>
...
<!-- jobcards / dnotes -->
@include('focus.boq_valuations.partials.jobcards')
    ...
    <div class="card-content">
        <div class="card-body">
            <ul class="nav nav-tabs nav-top-border nav-justified" role="tablist">
                ...
                <li class="nav-item">
                    <a class="nav-link h4 pt-1" id="active-tab3" data-toggle="tab" href="#active3" aria-controls="active3" role="tab" aria-selected="true">
                        Order Items
                    </a>
                </li>
                <li class="nav-item">
            ...
            <div class="tab-content px-1 pt-1 mb-1">
                <div class="tab-pane active in" id="active1" aria-labelledby="active-tab1" role="tabpanel">
                    <!-- Order Items -->
                    @include('focus.boq_valuations.partials.boq_items')
                    
                </div>
                <div class="tab-pane in" id="active3" aria-labelledby="active-tab3" role="tabpanel">
                    <!-- Order Items -->
                    @include('focus.boq_valuations.partials.order_items')
                    
                </div>
                <div class="tab-pane in" id="active2" aria-labelledby="active-tab2" role="tabpanel">
                    <!-- Material Expense -->
                    @include('focus.boq_valuations.partials.material_expense')
                    <!-- Direct Expense -->
                    @include('focus.boq_valuations.partials.service_expense')
                </div>
            </div>  
            <div class="row mb-1 no-gutters">
                <div class="col-2 mr-1">
                    <label for="retention" class="mr-5">% Retention</label>
            ...
            <!-- Valuation Summary -->
            @include('focus.boq_valuations.partials.summary')   
            @include('focus.boq_valuations.partials.upload_interim_cert')   
            
            <div class="edit-form-btn mt-1">
                {{ link_to_route('biller.boq_valuations.index', trans('buttons.general.cancel'), [], ['class' => 'btn btn-danger btn-md col-1 mr-2 ml-1']) }}
...
@include('focus.boq_valuations.form_js')
@endsection
