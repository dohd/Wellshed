            ...
            <div class="card-content">
                <div class="card-body">
                    {{ Form::model($purchase_requisition, ['route' => array('biller.purchase_requisitions.update', $purchase_requisition), 'method' => 'PATCH']) }}
                        @include('focus.purchase_requisitions.edit_form')
                    {{ Form::close() }}
                </div>
            </div>
        </div>
    ...
    const Form = {
        ...
        init() {
            ...
            if (this.purchaserequisition) {
                const requisition = this.purchaserequisition;
                $('#date').datepicker('setDate', new Date(requisition.date));
                $('#expect_date').datepicker('setDate', new Date(requisition.expect_date));
                $('#item_type').prop('disabled',true);
                if(requisition.item_type == 'project'){
                    ...
                    const projectName = "{{ $purchase_requisition->project? $purchase_requisition->project->name : '' }}";
                    const projectId = "{{ $purchase_requisition->project_id }}";
                    if (projectId) $('#project').append(new Option(projectName, projectId, true, true)).change();
                    const milestone_name = "{{ $purchase_requisition->milestone? $purchase_requisition->milestone->name : '' }}";
                    const milestone_id = "{{ $purchase_requisition->project_milestone_id }}";
