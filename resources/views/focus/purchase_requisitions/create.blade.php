...
<script>
    ...
    const Index = {
        ...
        drawDataTable() {
            $('#requisitionsTbl').dataTable({
                processing: true,
                serverSide: true,
                responsive: true,
                language: {@lang('datatable.strings')},
                ajax: {
                    url: "{{ route('biller.purchase_requisitions.get_requests') }}",
                    type: 'POST',
                },
                columns: [
                    {data: 'DT_Row_Index', name: 'id'},
                    ...['tid', 'status', 'date','project', 'note','employee', 'priority', 'expect_date'].map(v => ({data: v, name: v})),
