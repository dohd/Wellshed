            ...
            @foreach ($purchase_request->items as $k => $item)
                <tr>
                    @php
                        ...
                        $qty_remain = $item->qty - $item->total_qty_to_pr;
                    @endphp
                    <td><span class="numbering">{{$k+1}}</span></td>
                    <td><input type="text" name="product_name[]" id="name-p{{$k}}" value="{{$item->product_name}}" class="form-control" readonly></td>
                    ...
                    <td><span>{{numberFormat($item->qty)}}</span></td>
                    <td><input type="text" name="qty[]" id="qty-p{{$k}}" value="{{numberFormat($qty_remain)}}" class="form-control qty" readonly></td>
                    ...
                    <input type="hidden" name="purchase_request_item_id[]" class="purchase_request_item_id" value="{{$item->id}}">
                </tr>
            @endforeach
        @endisset
    </tbody>
