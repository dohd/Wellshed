...
<div class="card">
    <div class="card-content">
        <div class="card-body pb-0">
            <div class="form-group row">
                <div class="col-4">
                    ...
                    <select name="supplier_id" id="supplier" class="form-control" data-placeholder="Seach Supplier" required>
                        <option value=""></option>
                        @isset($creditnote)
                            <option value="{{ $creditnote->supplier_id }}" selected>
                                {{ $creditnote->supplier->company ?: $creditnote->supplier->name }}
                            </option>
                        @endisset
                    </select>                          
                </div>
            ...
            <div class="form-group row">
                <div class="col-2">
                    <label for="grn_type">Type</label>
                    <select name="grn_type" id="grn_type" class="form-control">
                        <option value="">--select type--</option>
                        <option value="grn_invoiced" {{ 'grn_invoiced' == @$creditnote->grn_type? 'selected' : '' }}>GRN with Invoice</option>
                        <option value="grn_not_invoiced" {{ 'grn_not_invoiced' == @$creditnote->grn_type? 'selected' : '' }}>GRNed not Invoiced</option>
                        <option value="vendor_credit" {{ 'vendor_credit' == @$creditnote->grn_type? 'selected' : '' }}>Vendor Credit</option>
                    </select>
                </div>
                <div class="col-4 div_grn">
                    <label for="grn">GRN</label>
                    <select name="grn_id" id="grn" class="form-control" data-placeholder="Choose GRN" required>
                        <option value=""></option>
                        @isset($creditnote->grn)
                            <option value="{{ $creditnote->grn_id }}" selected>
                                {{ gen4tid('GRN-', $creditnote->grn->tid) }} - {{ $creditnote->grn->note }}
                            </option>
                        @endisset
                    </select>
                    <input type="hidden" name="grn_id" id="grn_id" disabled>
                </div>
                <div class="col-4 d-none div_bill">
                    <label for="bill">Bill</label>
                    <select name="bill_id" id="bill" class="form-control" data-placeholder="Choose Bill" required>
                        <option value=""></option>
                        @isset($creditnote->bill)
                            <option value="{{ $creditnote->bill_id }}" selected>
                                {{ gen4tid('BILL-', $creditnote->bill->tid) }} - {{ $creditnote->bill->note }}
                            </option>
                        @endisset
                    </select>
                    <input type="hidden" name="bill_id" id="bill_id" disabled>
                </div>
                <div class="col-2">
                ...
                <div class="col-2">
                 ...
                 <div class="col-2 d-none div_amount">
                    <label for="amount">Amount</label>
                    <input type="text" id="amount" value="{{numberFormat(@$creditnote->total)}}" class="form-control">
                 </div>
            </div>
        ...
        <div class="card-body">
            {{-- <div class="row mb-1">
                <div class="col-2">
                    ...
                    <div class="form-check form-check-inline">
                        ...
                        <label class="form-check-label" for="inlineRadio1">Vendor Credit</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input radio-item" type="radio" name="load_items_from" id="radio-inv" value="1">
                        <label class="form-check-label" for="inlineRadio2">Bill</label>
                    </div>
                </div>
            </div> --}}
            ...
            <div class="table-responsive">
                <table id="products_tbl" class="table tfr my_stripe_single pb-1">
                    ...
                    <tbody>
                        @include('focus.supplier_creditnotes.partials.creditnote-item')
                    </tbody>
                </table>
            </div>
            <!--End Products Table -->
            ...
            <div class="row">
                <div class="col-2 ml-auto mr-auto">
                    <div class="edit-form-btn row">
                        {{ link_to_route('biller.supplier_creditnotes.index', trans('buttons.general.cancel'), $is_debit? compact('is_debit') : [], ['class' => 'btn btn-danger btn-md ml-auto mr-1']) }}
...
@include('focus.supplier_creditnotes.form_js')
@endsection