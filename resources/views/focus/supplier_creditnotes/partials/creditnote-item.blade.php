<tr class="d-none">
    ...
    <input type="hidden" class="inv-item-id" name="grn_item_id[]">
...
@if (@$creditnote)
    @foreach ($creditnote->items as $row)
        <tr>
                    ...
                    <div class="col-6">
                        <select class="custom-select prod-taxid" name='prod_taxid[]'>
                            ...
                            @foreach ($tax_rates as $tax_rate)
                                <option value="{{ $tax_rate->value }}" {{ $tax_rate->value == $row->prod_taxid? 'selected' : '' }}>
                                    {{ $tax_rate->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <input type="hidden" class="prod-subtotal" name="prod_subtotal[]">
            <input type="hidden" class="prod-taxable" name="prod_taxable[]">
            <input type="hidden" class="inv-item-id" name="grn_item_id[]" value="{{ $row->grn_item_id }}">
