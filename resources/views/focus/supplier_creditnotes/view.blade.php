...
<div class="content-wrapper">
    <div class="content-header row mb-1">
        <div class="content-header-left col-6">
            <h4>{{ $is_debit ? 'Debit Notes Management' : 'Supplier Credit Notes Management' }}</h4>
        </div>
                ...
                <div class="col-6">
                    <table id="customer-table" class="table table-sm table-bordered zero-configuration" cellspacing="0" width="100%">
                        <tbody>  
                            @php   
                                $details = [
                                    'Serial No.' => gen4tid($is_debit? 'DN-' : 'SCN-', $creditnote->tid),
                                    'Date' => dateFormat($creditnote->date),
                                    $creditnote->grn ? 'GRN No.' : 'BILL No.' => $creditnote->grn ? gen4tid('GRN-', @$creditnote->grn->tid) : gen4tid('BILL-', @$creditnote->bill->tid),
                                    'Supplier' => @$creditnote->supplier->company ?: @$creditnote->supplier->name,
                    ...
                    <tbody>
                        @foreach ($creditnote->items as $item)
                            <tr>
                                ...
                                <td>{{ numberFormat($item->prod_tax) }}</td>
                                <td>{{ numberFormat($item->prod_taxable) }}</td>
                                <td>{{ numberFormat($item->prod_total) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
