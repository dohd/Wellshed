<html>
<head>
	<title>{{ gen4tid('SCN-', $resource->tid) }}</title>
	<style>
		body {
	...
	<table width="100%" style="font-size: 10pt;margin-top:5px;">
		<tr>
			<td style="text-align: center;" width="100%" class="headerData">
				<span style="font-size:15pt; color:#0f4d9b;"><b>SUPPLIER CREDIT NOTE</b></span>
			</td>
		</tr>
	</table><br>
	<table width="100%" style="font-family: serif;font-size:10pt;" cellpadding="10">
		<tr>
			<td width="50%" style="border: 0.1mm solid #888888;"><span style="font-size: 7pt; color: #555555; font-family: sans;">SUPPLIER DETAILS:</span><br><br>
				<b>Supplier Name : </b>{{ $resource->supplier->company }}<br>
				<b>Supplier Tax Pin : </b>{{ $resource->supplier->taxid }}<br><br>
				<b>Address : </b>{{ $resource->supplier->address }}<br>
				<b>Email : </b>{{ $resource->supplier->email }}<br>
				<b>Cell : </b> {{ $resource->supplier->phone }}<br>
			<td width="5%">&nbsp;</td>
			<td width="45%" style="border: 0.1mm solid #888888;">
				<span style="font-size: 7pt; color: #555555; font-family: sans;">REFERENCE DETAILS:</span><br><br>
				<b>Credit Note No : </b> {{ gen4tid('SCN-', $resource->tid) }}<br>
				...
				<b>{{$resource->grn ? 'GRN No' : 'BILL No'}}: </b>{{ $resource->grn ? gen4tid('GRN-', $resource->grn->tid) : gen4tid('BILL-', $resource->bill->tid) }}<br>
			...
			@if ($resource->items->count())
				@foreach ($resource->items as $item)
					<tr class="dotted">
						...
						<td class="mytotalss" style="text-align: right;">{{ numberFormat($item->prod_subtotal) }}</td>                
					</tr>
				@endforeach
			@else
				<tr class="dotted">
