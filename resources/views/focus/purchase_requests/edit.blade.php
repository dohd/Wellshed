...
@section('title', 'Edit | Material Requisition Management')
    ...
    const Form = {
        ...
        init() {
            ...
            $('#requisitionsTbl').on('change', '.qty, .uom', function() {
                ...
                const milestone_id = row.find('.milestone_item').val();
                const qty_requested = accounting.unformat(row.find('.qty_requested').val());
                let total_qty = qty + qty_requested;
                let qty_dif = 0;
                
                if($('#item_type').val() == 'project'){
                    if(milestone_qty < total_qty){
                        if(milestone_qty == qty_requested && qty > qty_requested) {qty_diff = qty_requested}
                        else if(qty < qty_requested) {qty_diff = qty}
                        else {qty_diff = milestone_qty - qty_requested;}
                        console.log(qty_diff, milestone_id);
        ...
        stockRow(){
            return `
                <tr id="productRow">
                    ...
                    <td><input type="text" id="available_qty-p0" class="form-control" readonly></td>
        ...
        productRow(v,i){
            return `
                <tr>
                    ...
                    <td><input type="text" id="available_qty-p0" value="${v.available_qty}" class="form-control" readonly></td>
                    <td><input type="text" id="milestone_qty-p0" value="${v.qty}" class="form-control" readonly></td>
                    <td><input type="text" name="qty[]" id="qty-p0" value="${v.qty}" class="form-control"></td>
                    <td><button type="button" class="btn btn-danger delete"><i class="fa fa-minus-square" aria-hidden="true"></i></button></td>
