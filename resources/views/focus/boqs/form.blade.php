<div class='form-group row'>
    ...
    <div class="col-5">
        ...
        {{ Form::text('name', @$boqs->name, ['class' => 'form-control round', 'placeholder' => 'Title']) }}
    </div>
    <div class="col-3">
...
<div class="form-group row">
    @php
        $bq_total = 0;
        $total = $boqs->items()->sum(DB::raw('new_qty*boq_rate'));
        if($boqs->total_boq_amount > 1){
            $bq_total = $boqs->total_boq_amount;
        }else{
            $bq_total = $total;
        }
    @endphp
    ...
    <div class="col-3">
        {{ Form::label( 'bq_vat', 'Select Type of Vat',['class' => 'control-label']) }}
        <select name="vat_type" id="vat_type" class="form-control">
            <option value="">--select vat type--</option>
            <option value="inclusive" {{ @$boqs->vat_type == 'inclusive' ? 'selected' : '' }}>Boq figures are VAT Inclusive</option>
            <option value="exclusive" {{ @$boqs->vat_type == 'exclusive' ? 'selected' : '' }}>Boq figures do NOT have VAT</option>
        </select>
    </div>
    <div class="col-2 d-none">
        {{ Form::label( 'total_boq_vat', 'Total BoQ Vat',['class' => 'control-label']) }}
        {{ Form::text('total_boq_vat', numberFormat(@$boqs->total_boq_vat), ['class' => 'form-control round', 'placeholder' => '0.00','readonly']) }}
    </div>
    <div class="col-2">
        {{ Form::label( 'total_boq_amount', 'Total BoQ Amount',['class' => 'control-label']) }}
        {{ Form::text('total_boq_amount', numberFormat($bq_total), ['class' => 'form-control round', 'placeholder' => '0.00']) }}
    </div>
</div>

<div class="mt-3">
    @include('focus.boqs.partials.boq_items')
</div>
<div class="form-group row">
    <div class="col-6">
    ...
    <div class="col-3">
        ...
        <input type="hidden" name="boq_subtotal" id="boq_subtotal" class="form-control" readonly>
        <input type="hidden" name="boq_taxable" id="boq_vatable" class="form-control" readonly>
        {{-- <label class="mb-0" id="tax-label">{{ trans('general.total_tax') }}</label> --}}
        <input type="hidden" name="boq_tax" id="boq_tax" class="form-control" readonly>
        <label class="mb-0">BoQ Total
        </label>
        <input type="text" name="boq_total" class="form-control" id="boq_total" readonly>
    </div>
    <div class="col-3">
        <div>
