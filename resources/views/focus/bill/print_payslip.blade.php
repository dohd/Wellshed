...
<body>
    ...
    @php
        ...
        $generated_at = empty($resource->payroll->approval_date) ? '' : date("d-m-Y", strtotime($resource->payroll->approval_date));
    ...
    <h2 class="big_text">{{ gen4tid('PYRL-',@$resource->payroll->tid) }} <br>( {{ $monthName }} ) </h2>

    <table>
        <tr>
            <td>
                <span class="customer-dt-title">EMPLOYEE DETAILS</span><br><hr>
                ...
                <b>Contract Expiry Date :</b> {{ @$hrmmeta->employment_end_date ?? 'N/A' }}<br>
                <br>
