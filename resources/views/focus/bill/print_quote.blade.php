...
<body>
	...
	@if ($resource->productvar_id && $resource->appear_image == 'header')
	<table  class="customer-dt" cellpadding="10">
		<tr>
			<td width="45%">
				@php $image = "img/products/" . @$resource->productvar->image @endphp
				<img src="{{ is_file(Storage::disk('public')->path($image))? Storage::disk('public')->url($image) : '' }}" style="object-fit:contain" width="100%">
			</td>
			<td width="5%">&nbsp;</td>
			<td width="50%">
				<span class="customer-dt-title"><b>Specifications:</b></span><br><br>
				<p><b>{{@$resource->productvar->image_description}}</b></p>
			</td>
		</tr>
	</table><br>
	@endif
	<table class="items" cellpadding="8">
		...
		<tbody>
			...
			@foreach($resource->products as $product)
				...
				@else
					<tr>
						...
						@for($i = 0; $i < ($resource->uom_status == 'yes' ? 3 : 4); $i++) 
							<td></td>
						@endfor
					</tr>
					@php 
			...
			@endforeach
			<!-- Dynamic Empty Rows -->
			@for ($i = count($resource->products); $i < 10; $i++)
				<tr>
					@for($j = 0; $j < ($resource->uom_status == 'yes' ? 5 : 6); $j++) 
						<td></td>
					@endfor
				</tr>
			@endfor
			...
			<tr>
				<td colspan="{{ $resource->uom_status == 'yes' ? 3 : 4 }}" class="bd-t" rowspan="2">
					@isset($resource->bank)
					...
					@if ($resource->preparedBy)
						<em>Prepared By : </em><b>{{ $resource->preparedBy->fullname }}</b>
					@endif
				</td>
			...
			<tr>
				<td colspan="{{ $resource->uom_status == 'yes' ? 3 : 4 }}"></td>
	...
	@if ($resource->productvar_id && $resource->appear_image == 'footer')
	<table  class="customer-dt" cellpadding="10">
		<tr>
			<td width="45%">
				@php $image = "img/products/" . @$resource->productvar->image @endphp
