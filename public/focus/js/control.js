...
/**
 * Parse HTML to WhatsApp Text format
 * */
function htmlToWhatsAppFormat(htmlText) {
  const formatMap = {
    B: '*',         // <b> or <strong>
    STRONG: '*',
    I: '_',         // <i> or <em>
    EM: '_',
    STRIKE: '~',    // <strike> or <del>
    DEL: '~',
    CODE: '`'       // <code>
  };
  ...
  function traverse(node) {
    ...
    const wrapper = formatMap[tag];
    if (wrapper) {
      // Trim spaces inside formatting
...
function selectProjects(data) {
    ...
    $('#project_name').html('Project: <strong>' + data.project_name + '</strong>');
    $('#client_name').html('Client: <strong>' + data.client_name + '</strong>');
    $('#branch_name').html('Branch: <strong>' + data.branch_name + '</strong>');
    $('#project_number').html('Project Number: <strong>' + data.project_number + '</strong>');
  
    $("#projects-box").val();


    $("#projects").show();
    $("#projects-box-result").hide();
}
    ...
    if (errorNum > 0) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("html, body").scrollTop($("body").offset().top);
                $("#data_form").remove();
                if (trigger_n && typeof trigger == 'function') trigger(data);
                if (data.refresh) setTimeout(() => location.reload(), 1500);
            },
            error: function (data) {
                let message = '';
                if (trigger_n && typeof triggerError == 'function' && data.responseJSON.status == 'Error') {
...
function removeObject_c(input_data) {
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            $("#notify").removeClass("alert-danger").addClass("alert-success").fadeIn();
            $("html, body").scrollTop($("body").offset().top);
            if (input_data['trigger'] && typeof trigger == 'function') {
                trigger(data);
            }
        },
        error: function (data) {
            if (input_data['trigger'] && typeof triggerError == 'function' && data.responseJSON.status == 'Error') {
                data = data.responseJSON;
                triggerError(data);
                $("#notify .message").html("<strong>" + data.status + "</strong>: " + data.message);
                $("#notify").removeClass("alert-success").addClass("alert-warning").fadeIn();
