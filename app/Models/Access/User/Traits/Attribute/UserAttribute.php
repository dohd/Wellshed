...
trait UserAttribute
{
    ...
    {
        $has_package = TenantPackage::where('customer_id', auth()->user()->tenant_customer_id)
     ...
     * @return mixed
     */
    public function zgetPictureAttribute()
    {
    ...
    public function isLoginAccessActive()
    {
        return $this->login_access == 1;
    }
    ...
    {
        if ($this->id != 1) {
            return '<div class="btn-group p-1 dropup">
                        <button type="button" class="btn btn-sm btn-round btn-blue-grey dropdown-toggle" data-toggle="dropdown">
                               <i class="fa fa-cog"></i>
                        </button>
                        <ul class="dropdown-menu p-1 dropdown-menu-right">
    ...
    public function getActionButtonsAttribute()
    {
        ...
        if (access()->user()->roles[0]->all) {
            return '<div class="action-btn">
                    '.$this->getShowButtonAttribute('btn btn-success btn-round').' 
                    '.$this->getEditButtonAttribute('btn btn-warning btn-round').' 
                    '.$this->getChangePasswordButtonAttribute('btn btn-danger btn-round').'
                    '.$this->checkAdmin().'
                </div>';
        } else {
