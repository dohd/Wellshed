...
class SendKPIReport extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:my_kpi_reports';

    /**
     * The console Sending My KPI Reports.
     *
     * @var string
     */
    protected $description = 'Sending My KPI Reports';
    ...
    public function handle()
    {
        ...
        foreach ($companies as $company) {
            $first_year = FinancialYear::withoutGlobalScopes()->where('ins', $company->id)->latest()->first();
            ...
            $selectedMonth = now()->format('m'); // Month filter
            $pattern = '/^(0[17]\d{8}|254[17]\d{8})$/';
            $setting = RecipientSetting::withoutGlobalScopes()->where('type', 'kpi_report')->where('ins', $company->id)->first();
            foreach ($users as $user) {
                $employeeId = $user->id;    // Employee filter (user ID)
                ...
                if ($allocation) {
                    ...
                    $email_input = [
                        'text' => 'The link to your KPI report is ' . $link,
                    ...
                    $subject = "From {$company->sms_email_name}: Please find Link to your weekly or monthly KPI report " . $links;
                    $cost_per_160 = 0.6;
