...
class CompanyRepository extends BaseRepository
{
    ...
    public function __construct()
    {
        ...
        $this->file_footer_path = 'img' . DIRECTORY_SEPARATOR . 'company' . DIRECTORY_SEPARATOR;
    ...
    public function update(array $input)
    {
        ...
        $data['fiscal_month'] = date_for_database($data['fiscal_month']);

        // upload media
        if (@$data['logo']) $data['logo'] = $this->uploadPicture($input['data']['logo'], $this->file_picture_path);
        if (@$data['icon']) $data['icon'] = $this->uploadPicture($input['data']['icon'], $this->file_icon_path);
        if (@$data['theme_logo']) $data['theme_logo'] = $this->uploadPicture($input['data']['theme_logo'], $this->file_header_path);
        if (@$data['footer']) $data['footer'] = $this->uploadPicture($input['data']['footer'], $this->file_footer_path);
    ...
    */
    ...
    public function email_settings($data)
    {
        ...
        $company = EmailSetting::where('ins', '=', auth()->user()->ins)->first();
        if($company){
            $company->update(
                array(
                    'active' => $data['active'], 'host' => $data['host'], 
                    'port' => $data['port'], 'auth' => $data['auth'], 
                    'auth_type' => $data['auth_type'], 'username' => $data['username'],
                     'password' => $data['password'], 'sender' => $data['sender'],
                     'payslip_email_to' => $data['payslip_email_to'],
                    'customer_statement_email_to' => $data['customer_statement_email_to'],
                    'office_number' => $data['office_number'],
                    ));
        }else{
            EmailSetting::create(
                array(
                    'ins'=>auth()->user()->ins,'active' => $data['active'], 
                    'host' => $data['host'], 'port' => $data['port'], 'auth' => $data['auth'], 
                    'auth_type' => $data['auth_type'], 'username' => $data['username'], 
                    'password' => $data['password'], 'sender' => $data['sender'],
                    'payslip_email_to' => $data['payslip_email_to'],
                    'customer_statement_email_to' => $data['customer_statement_email_to'],
        ...
        $company_sms = SmsSetting::where('ins', '=', auth()->user()->ins)->first();
        if ($company_sms) {

            $company_sms->update(
                array(
                    'active' => $data['sms_active'], 'driver_id' => $data['s_driver_id'],
                    'driver' => $data['s_driver'], 'username' => $data['s_username'],
                    'password' => $data['s_password'], 'sender' => $data['s_sender']
                )
            );
        }else{
            SmsSetting::create(
