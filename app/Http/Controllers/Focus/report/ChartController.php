    ...
    public function chart(ManageReports $request)
    {
        ...
        switch ($request->interval) {
            ...
            case 'custom':
                ...
                $c['to_date'] = date_for_database($request->to_date);
                break;
        ...
        switch ($request->section) {
            case 'customer':
                ...
                if ($request->interval) {
                    ...
                    foreach ($chart_result as $row) {
                        $chart_array[] = array('y' => $row->customer['company'] ?? $row->customer['name'], 'a' => $row['amount']);
                    }
            ...
            case 'supplier':
                ...
                if ($request->interval) {
                    $chart_array = array();
                    foreach ($chart_result as $row) {
                        $chart_array[] = array('y' => $row->supplier['company'] ?? $row->supplier['name'], 'a' => $row['amount']);
                    }
                    return json_encode($chart_array);
                }
                return view('focus.chart.draw', compact('chart_result', 'lang'));
            ...
            case 'product':
                $chart_result = InvoiceItem::whereBetween('created_at', [datetime_for_database($c['from_date']), datetime_for_database($c['to_date'])])
                ->whereNotNull('product_id')
                ->groupBy('product_id')
                 ->select('description', DB::raw('sum(product_qty) as amount'))->orderBy('amount', 'desc')->take(100)->get();

                 $directProducts = InvoiceItem::select(
                    'product_id',
                    'description as description',
                    'product_qty as amount',
                    // DB::raw('SUM(product_amount) as amount')
                )
                ->whereNotNull('product_id')
                ->whereBetween('created_at', [datetime_for_database($c['from_date']), datetime_for_database($c['to_date'])])
                ->get();
                ...
                $quoteProducts = InvoiceItem::join('quotes', 'invoice_items.quote_id', '=', 'quotes.id')
                    ...
                    ->select(
                        ...
                        'verified_items.product_name as description',
                        'verified_items.product_qty as amount',
                    ...
                    ->whereBetween('invoice_items.created_at', [datetime_for_database($c['from_date']), datetime_for_database($c['to_date'])])
                    ->get();

                $lang['title'] = trans('meta.product_graphical_overview');
                $lang['module'] = 'product';
                if ($request->interval) {
                    $chart_result = $directProducts->concat($quoteProducts)
                    ->groupBy('product_id')
                    ->map(function ($group) {
                        return [
                    ...
                    $chart_array = array();
                    foreach ($chart_result as $row) {
                        $chart_array[] = array('y' => $row['description'], 'a' => $row['amount'],'label' => '');
                    }
                    return json_encode($chart_array);
                }
                return view('focus.chart.draw', compact('chart_result', 'lang'));
