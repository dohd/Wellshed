...
class BanksController extends Controller
{
    ...
    public function store(ManageCompanyRequest $request)
    {
        $request->validate([
            ...
            'number' => 'required',
            'feed_begin_balance' => request('feed_begin_balance')? 'required_with:feed_begin_date' : '',
            'feed_begin_date' => request('feed_begin_date')? 'required_with:feed_begin_balance' : '',
        ]);      
    ...
    public function update(ManageCompanyRequest $request, Bank $bank)
    {
        ...
        try {
            $this->repository->update($bank, $request->except(['_token', 'ins']));
        } catch (\Throwable $th) {
