...
class ChargesTableController extends Controller
{
    ...
    public function __invoke(ManageChargeRequest $request)
    {
        ...
        return Datatables::of($core)
            ->escapeColumns(['id'])
            ->addIndexColumn()
            ->editColumn('tid', function ($charge) {
                return gen4tid('CHRG-', $charge->tid);
            })
            ->addColumn('bank', function ($charge) {
                return @$charge->bank->holder;
            })
            ->addColumn('exp_account', function ($charge) {
                return @$charge->expense_account->holder;
            })
            ->addColumn('date', function ($charge) {
                return dateFormat($charge->date);
            })
            ->addColumn('amount', function ($charge) {
                return numberFormat($charge->amount);
            })
            ->editColumn('payment_mode', function ($charge) {
                $mode = $charge->payment_mode;
                if ($charge->reference) $mode = "{$charge->reference} - {$mode}";
                return $mode;
            })
